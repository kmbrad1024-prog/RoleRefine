# JD Optimizer — Prompt v2

The live prompt is in [`prompts.py`](../prompts.py). This file records what changed from v1 and why.

## Why v2

Testing v1 on real job postings found one serious failure and one smaller one:

1. **Dropped working conditions.** On a community organizer posting, the v1 rewrite removed "long hours and weekends", "ability to relocate", "working laptop", the daily office commute, the hourly pay type and the reporting line. It also cut the posting by more than half. The wording became gentler, but candidates would no longer know real parts of the job. That breaks the core rule: never change facts.
2. **Changed experience requirements.** On a sales posting, v1 lowered "2+ years" to fit the selected role level. Years of experience are the employer's decision, not a wording issue.

## What changed

| Area | v1 | v2 |
|---|---|---|
| Working conditions | Covered only by a general "never change facts" rule | New section: schedule, travel and relocation, on-site and commute terms, driving, pay type, employment type, required equipment, reporting line and deadlines must be kept. They can be reworded neutrally but not removed. |
| Experience years | "Use experience ranges only when justified by the role level" | Never change stated years; flag inflated ones in `suggestions` instead |
| Physical requirements | Remove if not essential | Remove only if *clearly* not essential; keep ones the duties need (e.g. driving for field outreach) |
| Removals | Not required to be logged | Every removed sentence or requirement must appear in `changes` |
| Length | ±20% | ±20%, plus "restructuring is fine; dropping facts to save space is not" |
| Invitations to apply | Not mentioned | Keep "we encourage you to apply…" and lists of welcome backgrounds; don't turn them into requirements |
| Few-shot example | Lowered "5+ years" to "3+ years" | Keeps "5+ years" and suggests reviewing it |

## Guardrail in code

v2 also adds [`factcheck.py`](../factcheck.py), which checks each rewrite without relying on the model. It compares the original and the rewrite for working conditions, numbers (years, pay, percentages, days) and the reporting line, and flags length changes above 25%. The app shows any warnings next to the result.

## Changelog

- v1: baseline.
- v2: working-conditions rule, keep experience years, log every removal, keep invitations to apply; code-level fact check.

## Full system prompt (v2)

```
You are JD Optimizer, an expert inclusive-hiring editor. You rewrite job descriptions so they appeal to the widest qualified talent pool, and you match the voice of the hiring company. You are a writing assistant, not a legal reviewer.
