...
**[Live demo →](https://YOUR-APP.streamlit.app)** · No API key needed; click **See a demo result**.
...
```
                ┌──────────────────────────┐
 JD + culture → │  Prompt (prompts.py)     │ → Claude → JSON: rewrite, changes, suggestions
                └──────────────────────────┘                     │
                                                                 ▼
...
- **Structured output:** strict JSON schema, validated in code, with an automatic retry if parsing fails.
...
Get an API key at [platform.claude.com](https://platform.claude.com). Without a key, the app runs in demo mode or lets each user paste their own.
...
3. In **Advanced settings → Secrets**, paste `ANTHROPIC_API_KEY = "sk-ant-..."`.
4. Deploy, then put the URL at the top of this README.

**Cost control:** each visitor gets 10 runs per session on your key (`MAX_RUNS_PER_SESSION` in `app.py`); after that they can paste their own key. Set a monthly spend limit in the Anthropic Console as a backstop.
...
llm.py              Claude API call, JSON validation, retry
...
- Built with [Claude](https://www.anthropic.com/claude) and [Streamlit](https://streamlit.io).
